...
Every transport-validated command turn SHALL have isolated bounded workflow state and use shared context, interpretation, validation, read/preparation and response behavior. Trusted actor, scope and dependencies SHALL come from the server and SHALL NOT be writable by interpreter output. Graph output SHALL end at an answer, clarification or sanitized preview; human confirmation SHALL use the existing application service through a separate transport-validated interaction. Polling and notifications SHALL remain independent of graph execution.
...
Memory SHALL hold the bounded queue, deduplication map, bindings, five-minute proposals, operation states, family slot, definition locks, and notification buffer. Initial limits SHALL be 100 queued commands, 10,000 deduplication entries retained up to 24 hours, 100 proposals, 100 unresolved mutation reservations/operations (including cancellation and automation), and 1,000 resolved operations retained up to 24 hours. Active or unknown operations SHALL NOT be evicted. Unresolved-mutation capacity SHALL be reserved before confirmation checks and released only for an owned uncommitted reservation or conclusively resolved operation. Saturation SHALL reject new mutations while bounded reads/diagnostics remain available; other full intake pools SHALL reject their own excess work. Accepted acknowledgement SHALL mean volatile receipt only.
...
#### Scenario: Accepted command later fails
- **WHEN** parsing, a read dependency or the command deadline fails after volatile acceptance
- **THEN** retained operation lookup SHALL expose a safe error and rejection without requiring replay; a previously prepared immutable preview SHALL remain recoverable with its original expiry.

#### Scenario: Unresolved mutation capacity is full
- **WHEN** unresolved operations consume the configured mutation budget
- **THEN** new mutations SHALL be rejected without eviction or another dispatch, while bounded reads and diagnostics remain available.

### Requirement: Process-local mutation admission
Admission SHALL be serialized and protect one bot-created primary family slot per installation/environment. Immediately before invocation, targeted authorization, target, approved inputs, code identity and retry checks SHALL be no older than five seconds. Complete historical discovery MAY span bounded passes; it SHALL finish before these fresh targeted checks and incomplete coverage SHALL defer launch. Preparation SHALL consume completed discovery or return INCOMPLETE_EVIDENCE within its turn budget, not await a multi-pass scan inside the workflow. Materially changed evidence SHALL invalidate the preview, except verified already-terminal cancellation or already-desired automation state, which SHALL complete without sending a mutation and invalidate its controls. Competing launches SHALL receive busy responses without deferred automatic execution; reads SHALL remain available. Cancellation and automation SHALL require their own approvals and definition/target serialization.

Checks SHALL inspect existing request matches, source attempts across both rerun and fresh-copy modes, and scoped active or pending families including retry gaps. Queries SHALL exhaust required cursor pagination within bounded budgets; truncated, inaccessible, or ambiguous evidence SHALL defer mutations. Neither the latest run, first page, nor a recent time window SHALL prove absence. Proven prior requests SHALL return their existing result; repeating a completed attempt SHALL require a new preview and explicit repeat acknowledgement through a repeat-labeled confirmation bound to the unchanged prior-attempt evidence. Active, pending, or unknown attempts SHALL block parallel retry.
